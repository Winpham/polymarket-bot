...
def mkey(r):
    # An explicit "cluster" on the row wins (weather clusters by resolution DAY — cross-city
    # same-day temperature is correlated, so the super-key over-counts independence). Else the
    # match super-key (a best-of-3's map/series markets are ONE bet), else the market id.
    if r.get("cluster"):
        return r["cluster"]
